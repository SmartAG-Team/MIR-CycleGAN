...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input):
        ...
        intermediate_input_01 = torch.cat((output_0, output_1), 1)
        intermediate_input_02 = torch.cat((output_0, output_1), 1)
        intermediate_input_12 = torch.cat((output_1, output_0), 1)
...
class UnetIndependentBranchedSkipConnectionBlockDown(nn.Module):
    ...
    def forward(self, x):
        if self.innermost:
            return torch.cat([self.model_0(x[0]), self.model_1(x[1])], 1)
        return [self.model_0(x[0]), self.model_1(x[1])]
...
class UnetBranchedSkipConnectionBlockDown(nn.Module):
    ...
    def forward(self, x):
        if self.innermost:
            return torch.cat([self.model(x[0]), self.model(x[1])], 1)
...
class UnetBranchedSkipConnectionBlockUp(nn.Module):
    ...
    def forward(self, x1, x2):
        if self.outermost:
            ...
            return torch.cat([x2[0], self.model(x1)], 1)
