...
from utils import *
from utils import set_requires_grad, GANLoss, get_rgb_stgan
import warnings
import os
...
class Conv4(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Conv2d(3, 4, kernel_size=1)  # 输入通道数为3，输出通道数为4的卷积层
...
def train(opt, model_GEN, model_DIS, cloud_detection_model, optimizer_G, optimizer_D, train_loader, val_loader):
    writer = SummaryWriter('runs34/%s' % opt.dataset_name)
    ...
    for epoch in range(opt.n_epochs):
        ...
        for real_A, real_B, _ in train_loader:
            # real_A[0] = model(real_A[0])
            # real_A[1] = model(real_A[1])
            # real_A[2] = model(real_A[2])
            # real_B = model(real_B)
            ...
            if opt.aux_loss:
                loss_G_aux = (criterionL1(aux_pred[0][:, :3, ...], real_B) + criterionL1(
                    aux_pred[1][:, :3, ...], real_B) + criterionL1(aux_pred[2][:, :3, ...], real_B)) * opt.lambda_aux
...
def valid(opt, model_GEN, val_loader, criterionL1, writer, epoch):
    ...
    pbar = tqdm.tqdm(
        ...
        ncols=0,
        desc="Valid[%d/%d]" % (epoch, opt.n_epochs),
        unit=" step",
    )
    model = Conv4()
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    """Path"""
    parser.add_argument("--root", type=str, default='/root/autodl-tmp/PMAA',
                        help="Path to dataset")
    parser.add_argument("--cloud_model_path", type=str,
                        default='/home/projects/PMAA-main/data/Feature_Extrator_FS2.pth', help="path to feature extractor model")
    parser.add_argument("--save_model_path", type=str,
                        default='/root/autodl-tmp/PMAA/checkpoints', help="Path to save model")
    parser.add_argument("--dataset_name", type=str,
                        default='PMAA', help="name of the dataset")
    ...
    total_data = MultipleDataset(
        root=os.path.join(opt.root, "dataset_short"),
        band=opt.in_channel,
    )
    train_data, val_data, _ = random_split(
        dataset=total_data,
        # lengths=(7000, 1000, 1000),
        # lengths=(1, 1, 1),
        lengths=(2000, 500, 500),
        generator=torch.Generator().manual_seed(opt.manual_seed),
    )
    ...
    DIS = Discriminator(input_nc=int(3*4+3))
